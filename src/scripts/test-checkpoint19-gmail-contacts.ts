...
async function main() {
  // ---------- A: "Draft an email to Alice saying hello" -> resolves, drafts, does NOT send ----------
  {
    ...
    const r = await runTask({ goal: 'Draft an email to Alice saying hello', onEvent: () => {}, taskId: nanoid() });
    check(
  ...
  {
    pendingActionStore.clear();
    const r = await runTask({ goal: 'Draft an email to John Smith saying are you free today', onEvent: () => {}, taskId: nanoid() });
    check(
      'B. ambiguous contact -> clarification shown, no draft created, no pending action',
      r.outcome === 'blocked' && /Which one/i.test(r.result) && !pendingActionStore.active(),
      `outcome=${r.outcome} result=${r.result.slice(0, 200)} pending=${!!pendingActionStore.active()}`
    );
  }
  ...
  {
    pendingActionStore.clear();
    const r = await runTask({ goal: 'Draft an email to Zorblax saying hello', onEvent: () => {}, taskId: nanoid() });
    check(
      'C. unknown name -> clarification asking for explicit email, no draft, no pending action',
      r.outcome === 'blocked' && /couldn't find/i.test(r.result) && !pendingActionStore.active(),
  ...
  {
    pendingActionStore.clear();
    const r = await runTask({ goal: 'Draft an email to explicit@example.com saying hello', onEvent: () => {}, taskId: nanoid() });
    check(
  ...
  {
    pendingActionStore.clear();
    await runTask({ goal: 'Draft an email to Alice saying duplicate protection check', onEvent: () => {}, taskId: nanoid() });
