...
async function main() {
  ...
  {
    const r = await runTask({ sessionId: SID, goal: 'Jarvis, draft an email to test@example.com saying I will join at three.', onEvent: () => {}, taskId: nanoid() });
    check(
  ...
  {
    const r = await runTask({ sessionId: SID,
  ...
  {
    const r = await runTask({ sessionId: SID, goal: 'Draft an email to notanemail saying hello.', onEvent: () => {}, taskId: nanoid() });
    check(
    ...
    check('D. invalid recipient — no pending send action was created', !pendingActionStore.active(SID));
  }

  // ---------- E: missing recipient -> clarification, not a guess ----------
  {
    const r = await runTask({ sessionId: SID, goal: 'Draft an email to Alex saying I will join at three.', onEvent: () => {}, taskId: nanoid() });
    check(
    ...
    check('E. missing recipient — no draft/pending action created', !pendingActionStore.active(SID));
  }
