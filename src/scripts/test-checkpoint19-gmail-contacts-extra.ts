...
async function main() {
  // ---------- D: contact with two emails, neither marked primary -> clarification, zero drafts ----------
  {
    ...
    const r = await runTask({ goal: 'Draft an email to Sam saying are we still on for lunch', onEvent: () => {}, taskId: nanoid() });
    check(
      'D. multi-email contact with no primary -> clarification, no draft, no pending action',
      r.outcome === 'blocked' && /use .*sam\.work@example\.com.* or .*sam\.personal@example\.com/i.test(r.result) && !pendingActionStore.active(),
  ...
  // ---------- H: cancellation mid-lookup -> stopped, no draft, no pending action ----------
  {
    ...
    controller.abort();
    const r = await runTask({ goal: 'Draft an email to Alice saying hello', onEvent: () => {}, signal: controller.signal, taskId: nanoid() });
    check(
      'H. cancelled before Contacts lookup resolves -> stopped, no draft, no pending action',
      r.status === 'stopped' && !pendingActionStore.active(),
      `status=${r.status} result=${r.result.slice(0, 200)} pendingActive=${!!pendingActionStore.active()}`
    );
  }
