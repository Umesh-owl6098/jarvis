...
async function main() {
  ...
  {
    const r = await runTask({ goal: 'Show my latest emails', onEvent: () => {}, taskId: nanoid() });
    check(
  ...
  // ---------- B: search by sender ----------
  {
    const r = await runTask({ goal: 'Find the email from John', onEvent: () => {}, taskId: nanoid() });
    check(
  ...
  {
    const r = await runTask({ goal: 'Search my email for the invoice', onEvent: () => {}, taskId: nanoid() });
    check(
  ...
  // ---------- D: read one thread ----------
  {
    const r = await runTask({ goal: 'Read my latest thread with Sarah', onEvent: () => {}, taskId: nanoid() });
    check(
  ...
  {
    const r = await runTask({ goal: 'Summarize my latest thread with Sarah', onEvent: () => {}, taskId: nanoid() });
    check(
  ...
  {
    const r = await runTask({ goal: 'Open gmail.com', onEvent: () => {}, taskId: nanoid() });
    check(
