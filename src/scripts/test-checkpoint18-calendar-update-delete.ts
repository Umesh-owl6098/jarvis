...
async function main() {
  // ---------- G: update proposal + confirm ----------
  {
    ...
    const proposeResult = await runTask({ sessionId: SID, goal: 'Move my 3 PM meeting to 5 PM.', onEvent: () => {}, taskId: nanoid() });
    check(
    ...
    const confirmResult = await runTask({ sessionId: SID, goal: 'Update it.', onEvent: () => {}, taskId: nanoid() });
    check(
  ...
  // ---------- H: delete proposal + confirm ----------
  {
    ...
    const proposeResult = await runTask({ sessionId: SID, goal: 'Cancel my Quarterly Planning meeting.', onEvent: () => {}, taskId: nanoid() });
    check(
    ...
    const confirmResult = await runTask({ sessionId: SID, goal: 'Cancel it.', onEvent: () => {}, taskId: nanoid() });
  ...
  // ---------- I: ambiguous event search — multiple matches asks for clarification, never guesses ----------
  {
    ...
    const r = await runTask({ sessionId: SID, goal: 'Cancel my Ambiguous Sync meeting.', onEvent: () => {}, taskId: nanoid() });
    check(
      'I. ambiguous search — multiple matches asks for clarification, no pending action created, nothing deleted',
      r.outcome === 'blocked' && /Multiple events match/i.test(r.result) && !calendarPendingActionStore.active(SID),
